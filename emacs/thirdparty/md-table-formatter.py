# coding: utf-8
# Ref: https://github.com/qwerqwer1111/md-table-formatter
...
def _decode(cur_str, encoding):
    if sys.version_info[0] >= 3:
        return cur_str

    return cur_str.decode(encoding)


def _encode(cur_str, encoding):
    if sys.version_info[0] >= 3:
        return cur_str

    return cur_str.encode(encoding)
...
def main():
    ...
    lines = [
        _decode(line, encoding) if isinstance(line, unicode) else line for line in lines
    ]
    ...
    formatted_lines = [
        _encode(line, encoding) if isinstance(line, str) else line
